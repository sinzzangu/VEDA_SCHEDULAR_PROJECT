 ...
 *
 * [역할]
 *   프로젝트 추가/삭제를 한 곳에서 관리하는 "프로젝트 관리 센터" 다이얼로그.
 *   상단에 이름 입력 + 추가 버튼, 하단에 기존 프로젝트 목록과 각 행별
 *   삭제 버튼을 제공하여 한 화면에서 CRUD 흐름을 완결함.
 *
 * [주요 기능]
 *   - QListWidget 기반 프로젝트 목록 렌더링
 *     (각 row: 폴더 아이콘 + 이름 + 일정 개수 + 삭제 버튼 커스텀 위젯)
 *   - 이름 입력 후 "+ 추가" 시 빈값/중복 검사 후 추가 요청 시그널 발생
 *   - 각 row 삭제 버튼 클릭 시 확인 다이얼로그 후 삭제 요청 시그널 발생
 *   - refresh_project_list로 외부 데이터 변경 후 목록 즉시 갱신
 *
 * Created By 방준한
 * Version 2.0 (단순 추가용 → 관리 센터 패턴으로 확장)
 ***********************************************************************/
